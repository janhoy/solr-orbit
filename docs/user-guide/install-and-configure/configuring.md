...
# Configuring Apache Solr Orbit

Apache Solr Orbit stores its configuration in `~/.solr-orbit/benchmark.ini`, which is
automatically created the first time Solr Orbit runs.
...
| :---- | :---- | :---- |
| `config.version` | Integer | The version of the configuration file format. This value is managed by Solr Orbit and should not be changed manually. |

## system

This section contains global information for the current benchmark environment. These settings
should be identical on all machines where Solr Orbit is installed.
...
| `available.cores` | Integer | The number of available CPU cores. Solr Orbit aims to create one asyncio event loop per core and distributes clients evenly. Defaults to the number of logical CPU cores on the host. |
| `async.debug` | Boolean | Enables debug mode on Solr Orbit's asyncio event loop. Default is `false`. |
...
| Parameter | Type | Description |
| :---- | :---- | :---- |
| `root.dir` | String | The directory that stores all Solr Orbit data. Solr Orbit assumes full control over this directory and all its subdirectories. Default is `~/.solr-orbit`. |

## benchmarks

This section configures the Solr Orbit data directory.

| Parameter | Type | Description |
| :---- | :---- | :---- |
| `local.dataset.cache` | String | The directory in which benchmark datasets are stored. Depending on the workloads you run, this directory may contain hundreds of GB of data. Default is `~/.solr-orbit/benchmarks/data`. |
...
| `sample.queue.size` | Integer | The number of metric samples that can be held in Solr Orbit's in-memory queue. Default is `2^20`. |
...
`~/.solr-orbit/benchmarks/test-runs/<run-id>/metrics.jsonl` incrementally during the run.
...
This section defines the default values for certain Solr Orbit CLI parameters.
...
If your host accesses the internet through an HTTP proxy, you can configure Solr Orbit to
...
4. Verify the proxy is working by cloning the workloads repository:
   ...
   ```bash
   git clone https://github.com/apache/solr-orbit-workloads.git
   ```

5. Confirm Solr Orbit picks up the proxy setting by checking the log at
   `~/.solr-orbit/logs/benchmark.log`. At startup you should see:
...
Logs from Solr Orbit can be configured in `~/.solr-orbit/logging.json`. For details on
...
By default, Solr Orbit writes all log output to `~/.solr-orbit/logs/benchmark.log`.
Log level is controlled via `~/.solr-orbit/logging.json`, not via a CLI flag.
