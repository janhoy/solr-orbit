...
def http_proxy():
    ...
    lines = process.run_subprocess_with_output(
        f"docker run --rm --name squid -d -v {config_dir}/squidpasswords:/etc/squid/squidpasswords -v {config_dir}/squid.conf:/etc/squid/squid.conf -p 3128:3128 ubuntu/squid"
    )
