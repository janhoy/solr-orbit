...
class InMemoryMetricsStoreTests(TestCase):
    ...
    def test_get_one(self):
        ...
        self.metrics_store.open(InMemoryMetricsStoreTests.TEST_RUN_ID, InMemoryMetricsStoreTests.TEST_RUN_TIMESTAMP, "test", "append-no-conflicts", "defaults", create=True)
        ...
        self.metrics_store.open(InMemoryMetricsStoreTests.TEST_RUN_ID, InMemoryMetricsStoreTests.TEST_RUN_TIMESTAMP, "test", "append-no-conflicts", "defaults")

        actual_duration = self.metrics_store.get_one("service_time", task="task1", mapper=lambda doc: doc["relative-time-ms"], sort_key="relative-time-ms", sort_reverse=True)
    ...
    def test_out_of_memory(self, virt_mem, console_warn):
        ...
        self.metrics_store.put_value_cluster_level("indexing_throughput", throughput, "docs/s")
        self.metrics_store.put_value_cluster_level("final_index_size", 1000, "GB")
        console_warn.assert_has_calls([mock.call("Memory threshold exceeded by in-memory metrics store, not adding additional entries", logger=mock.ANY)])
        self.metrics_store.to_externalizable(clear=True)
        console_warn.assert_has_calls([mock.call("Memory threshold exceeded by in-memory metrics store, skipping summary generation for current operation", logger=mock.ANY)])
        self.metrics_store.close()
    ...
    def assert_equal_percentiles(self, name, percentiles, expected_percentiles):
        ...
        for percentile, actual_percentile_value in actual_percentiles.items():
            self.assertAlmostEqual(expected_percentiles[percentile], actual_percentile_value, places=1, msg=str(percentile) + "th percentile differs")

    def test_filter_percentiles_by_sample_size(self):
        test_percentiles = [0, 0.0001, 0.001, 0.01, 0.1, 4, 10, 10.01, 25, 45, 46.001, 50, 75, 80.1, 90, 98.9, 98.91, 98.999, 99, 99.9, 99.99, 99.999, 99.9999, 100]
        sample_size_to_result_map = {
            ...
            1000000: [0, 0.0001, 0.001, 0.01, 0.1, 4, 10, 10.01, 25, 45, 46.001, 50, 75, 80.1, 90, 98.9, 98.91, 98.999, 99, 99.9, 99.99, 99.999, 99.9999, 100],
    ...
    def test_get_error_rate_by_sample_type(self):
        self.metrics_store.open(InMemoryMetricsStoreTests.TEST_RUN_ID, InMemoryMetricsStoreTests.TEST_RUN_TIMESTAMP, "test", "append-no-conflicts", "defaults", create=True)
        self.metrics_store.put_value_cluster_level("service_time", 3.0, "ms", task="term-query", sample_type=metrics.SampleType.Warmup, meta_data={"success": False})
    ...
    def test_get_error_rate_mixed(self):
        ...
        self.metrics_store.put_value_cluster_level("service_time", 3.0, "ms", task="term-query", sample_type=metrics.SampleType.Normal, meta_data={"success": True})
        self.metrics_store.put_value_cluster_level("service_time", 3.0, "ms", task="term-query", sample_type=metrics.SampleType.Normal, meta_data={"success": True})
        self.metrics_store.put_value_cluster_level("service_time", 3.0, "ms", task="term-query", sample_type=metrics.SampleType.Normal, meta_data={"success": False})
        self.metrics_store.put_value_cluster_level("service_time", 3.0, "ms", task="term-query", sample_type=metrics.SampleType.Normal, meta_data={"success": True})
        self.metrics_store.put_value_cluster_level("service_time", 3.0, "ms", task="term-query", sample_type=metrics.SampleType.Normal, meta_data={"success": True})
...
class FileTestRunStoreTests(TestCase):
    ...
    def test_store_test_run(self):
        schedule = [workload.Task("index #1", workload.Operation("index", workload.OperationType.Bulk))]

        t = workload.Workload(
            name="unittest", collections=[workload.Collection(name="tests")], test_procedures=[workload.TestProcedure(name="index", default=True, schedule=schedule)]
        )

        test_run = metrics.TestRun(
            benchmark_version="0.4.4",
            benchmark_revision="123abc",
            ...
            results=FileTestRunStoreTests.DictHolder(
                {
                    "young_gc_time": 100,
                    "old_gc_time": 5,
                    "op_metrics": [{"task": "index #1", "operation": "index", "throughput": {"min": 1000, "median": 1250, "max": 1500, "unit": "docs/s"}}],
                }
            ),
        )
        ...
        retrieved_test_run = self.test_run_store.find_by_test_run_id(test_run_id=FileTestRunStoreTests.TEST_RUN_ID)
...
class StatsCalculatorTests(TestCase):
    def test_calculate_global_stats(self):
        ...
        store.put_value_cluster_level("latency", 2800, unit="ms", task="index #1", operation_type=workload.OperationType.Bulk, sample_type=metrics.SampleType.Warmup)
        ...
        store.put_value_cluster_level("service_time", 190, unit="ms", task="index #1", operation_type=workload.OperationType.Bulk, meta_data={"success": True}, relative_time=595)
        store.put_value_cluster_level("service_time", 200, unit="ms", task="index #1", operation_type=workload.OperationType.Bulk, meta_data={"success": False}, relative_time=709)
        store.put_value_cluster_level("service_time", 210, unit="ms", task="index #1", operation_type=workload.OperationType.Bulk, meta_data={"success": True}, relative_time=653)

        # only warmup samples
        store.put_value_cluster_level("throughput", 500, unit="docs/s", task="index #2", sample_type=metrics.SampleType.Warmup, operation_type=workload.OperationType.Bulk)
        store.put_value_cluster_level("latency", 2800, unit="ms", task="index #2", operation_type=workload.OperationType.Bulk, sample_type=metrics.SampleType.Warmup)
        store.put_value_cluster_level(
            "service_time", 250, unit="ms", task="index #2", operation_type=workload.OperationType.Bulk, sample_type=metrics.SampleType.Warmup, relative_time=600
        )

        store.put_doc(
            doc={"name": "ml_processing_time", "job": "benchmark_ml_job_1", "min": 2.2, "mean": 12.3, "median": 17.2, "max": 36.0, "unit": "ms"},
        ...
        self.assertEqual(collections.OrderedDict([("min", 500), ("mean", 1125), ("median", 1000), ("max", 2000), ("unit", "docs/s")]), opm["throughput"])
        ...
        self.assertEqual(collections.OrderedDict([("min", None), ("mean", None), ("median", None), ("max", None), ("unit", "docs/s")]), opm2["throughput"])
    ...
    def test_calculate_system_stats(self):
        ...
        self.assertEqual([{"node": "benchmark-node-0", "name": "index_size", "value": 2048, "unit": "bytes"}], stats.node_metrics)
...
class GlobalStatsCalculatorTests(TestCase):
    ...
    def test_add_administrative_task_with_error_rate_in_results(self):
        ...
        self.metrics_store.open(InMemoryMetricsStoreTests.TEST_RUN_ID, InMemoryMetricsStoreTests.TEST_RUN_TIMESTAMP, "test", "append-fast-with-conflicts", "defaults", create=True)
        self.metrics_store.put_doc(
            doc={
                ...
                "test-run-timestamp": "20200728T003905Z",
                "environment": "local",
                "workload": "geonames",
                "test_procedure": "append-fast-with-conflicts",
                "cluster-config-instance": "defaults",
                "name": "service_time",
                "value": 72.67997100007051,
                "unit": "ms",
                "sample-type": "normal",
                "meta": {"source_revision": "7f634e9f44834fbc12724506cc1da681b0c3b1e3", "distribution_version": "7.6.0", "distribution_flavor": "oss", "success": False},
                "task": "delete-index",
        ...
        result = GlobalStatsCalculator(store=self.metrics_store, workload=Workload(name="geonames", meta_data={}), test_procedure=test_procedure)()
...
class GlobalStatsTests(TestCase):
    def test_as_flat_list(self):
        d = {
            "op_metrics": [
                {
                    "task": "index #1",
                    "operation": "index",
                    "throughput": {"min": 450, "mean": 450, "median": 450, "max": 452, "unit": "docs/s"},
                    "latency": {
                        "50": 340,
                        "100": 376,
                    },
                    "service_time": {"50": 341, "100": 376},
                    "error_rate": 0.0,
                    "meta": {"clients": 8, "phase": "idx"},
                },
                {
                    "task": "search #2",
                    "operation": "search",
                    "throughput": {"min": 9, "mean": 10, "median": 10, "max": 12, "unit": "ops/s"},
                    "latency": {
                        "50": 99,
                        "100": 111,
                    },
                    "service_time": {"50": 98, "100": 110},
                    "error_rate": 0.1,
                },
            ],
            "ml_processing_time": [
                {"job": "job_1", "min": 3.3, "mean": 5.2, "median": 5.8, "max": 12.34},
                {"job": "job_2", "min": 3.55, "mean": 4.2, "median": 4.9, "max": 9.4},
            ],
            "young_gc_time": 68,
            "young_gc_count": 7,
            "old_gc_time": 0,
            "old_gc_count": 0,
            "merge_time": 3702,
            "merge_time_per_shard": {"min": 40, "median": 3702, "max": 3900, "unit": "ms"},
            "merge_count": 2,
            "refresh_time": 596,
            "refresh_time_per_shard": {"min": 48, "median": 89, "max": 204, "unit": "ms"},
            "refresh_count": 10,
            "flush_time": None,
        ...
        self.assertEqual(
            {
                ...
                "task": "index #1",
                "operation": "index",
                "value": {"min": 450, "mean": 450, "median": 450, "max": 452, "unit": "docs/s"},
                "meta": {"clients": 8, "phase": "idx"},
            },
        ...
        self.assertEqual(
            {"name": "service_time", "task": "index #1", "operation": "index", "value": {"50": 341, "100": 376}, "meta": {"clients": 8, "phase": "idx"}},
            select(metric_list, "service_time", operation="index"),
        )

        self.assertEqual(
            {"name": "latency", "task": "index #1", "operation": "index", "value": {"50": 340, "100": 376}, "meta": {"clients": 8, "phase": "idx"}},
            select(metric_list, "latency", operation="index"),
        )

        self.assertEqual(
            {"name": "error_rate", "task": "index #1", "operation": "index", "value": {"single": 0.0}, "meta": {"clients": 8, "phase": "idx"}},
            select(metric_list, "error_rate", operation="index"),
        )

        self.assertEqual(
            {"name": "throughput", "task": "search #2", "operation": "search", "value": {"min": 9, "mean": 10, "median": 10, "max": 12, "unit": "ops/s"}},
            select(metric_list, "throughput", operation="search"),
        )

        self.assertEqual(
            {"name": "service_time", "task": "search #2", "operation": "search", "value": {"50": 98, "100": 110}}, select(metric_list, "service_time", operation="search")
        )

        self.assertEqual({"name": "latency", "task": "search #2", "operation": "search", "value": {"50": 99, "100": 111}}, select(metric_list, "latency", operation="search"))

        self.assertEqual({"name": "error_rate", "task": "search #2", "operation": "search", "value": {"single": 0.1}}, select(metric_list, "error_rate", operation="search"))

        self.assertEqual(
            {"name": "ml_processing_time", "job": "job_1", "value": {"min": 3.3, "mean": 5.2, "median": 5.8, "max": 12.34}}, select(metric_list, "ml_processing_time", job="job_1")
        )

        self.assertEqual(
            {"name": "ml_processing_time", "job": "job_2", "value": {"min": 3.55, "mean": 4.2, "median": 4.9, "max": 9.4}}, select(metric_list, "ml_processing_time", job="job_2")
        )

        self.assertEqual({"name": "young_gc_time", "value": {"single": 68}}, select(metric_list, "young_gc_time"))
        self.assertEqual({"name": "young_gc_count", "value": {"single": 7}}, select(metric_list, "young_gc_count"))

        self.assertEqual({"name": "old_gc_time", "value": {"single": 0}}, select(metric_list, "old_gc_time"))
        self.assertEqual({"name": "old_gc_count", "value": {"single": 0}}, select(metric_list, "old_gc_count"))

        self.assertEqual({"name": "merge_time", "value": {"single": 3702}}, select(metric_list, "merge_time"))

        self.assertEqual({"name": "merge_time_per_shard", "value": {"min": 40, "median": 3702, "max": 3900, "unit": "ms"}}, select(metric_list, "merge_time_per_shard"))

        self.assertEqual({"name": "merge_count", "value": {"single": 2}}, select(metric_list, "merge_count"))

        self.assertEqual({"name": "refresh_time", "value": {"single": 596}}, select(metric_list, "refresh_time"))

        self.assertEqual({"name": "refresh_time_per_shard", "value": {"min": 48, "median": 89, "max": 204, "unit": "ms"}}, select(metric_list, "refresh_time_per_shard"))

        self.assertEqual({"name": "refresh_count", "value": {"single": 10}}, select(metric_list, "refresh_count"))

        self.assertIsNone(select(metric_list, "flush_time"))
        self.assertIsNone(select(metric_list, "flush_time_per_shard"))
        self.assertEqual({"name": "flush_count", "value": {"single": 0}}, select(metric_list, "flush_count"))
...
class SystemStatsTests(TestCase):
    def test_as_flat_list(self):
        d = {
            "node_metrics": [
                {"node": "benchmark-node-0", "name": "startup_time", "value": 3.4},
                {"node": "benchmark-node-1", "name": "startup_time", "value": 4.2},
                {"node": "benchmark-node-0", "name": "index_size", "value": 300 * 1024 * 1024},
                {"node": "benchmark-node-1", "name": "index_size", "value": 302 * 1024 * 1024},
                {"node": "benchmark-node-0", "name": "bytes_written", "value": 817 * 1024 * 1024},
                {"node": "benchmark-node-1", "name": "bytes_written", "value": 833 * 1024 * 1024},
            ],
        }
        ...
        self.assertEqual({"node": "benchmark-node-0", "name": "startup_time", "value": {"single": 3.4}}, select(metric_list, "startup_time", node="benchmark-node-0"))

        self.assertEqual({"node": "benchmark-node-1", "name": "startup_time", "value": {"single": 4.2}}, select(metric_list, "startup_time", node="benchmark-node-1"))

        self.assertEqual({"node": "benchmark-node-0", "name": "index_size", "value": {"single": 300 * 1024 * 1024}}, select(metric_list, "index_size", node="benchmark-node-0"))

        self.assertEqual({"node": "benchmark-node-1", "name": "index_size", "value": {"single": 302 * 1024 * 1024}}, select(metric_list, "index_size", node="benchmark-node-1"))

        self.assertEqual(
            {"node": "benchmark-node-0", "name": "bytes_written", "value": {"single": 817 * 1024 * 1024}}, select(metric_list, "bytes_written", node="benchmark-node-0")
        )

        self.assertEqual(
            {"node": "benchmark-node-1", "name": "bytes_written", "value": {"single": 833 * 1024 * 1024}}, select(metric_list, "bytes_written", node="benchmark-node-1")
        )
