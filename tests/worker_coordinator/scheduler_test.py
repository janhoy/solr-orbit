...
class SchedulerTestCase(TestCase):
    ...
    def assertThroughputEquals(self, sched, expected_average_throughput, msg="", relative_delta=0.05):
        ...
        self.assertGreaterEqual(actual_average_rate, expected_lower_bound, f"{msg}: expected target rate to be >= [{expected_lower_bound}] but was [{actual_average_rate}].")
        self.assertLessEqual(actual_average_rate, expected_upper_bound, f"{msg}: expected target rate to be <= [{expected_upper_bound}] but was [{actual_average_rate}].")
...
class UnitAwareSchedulerTests(TestCase):
    def test_scheduler_rejects_differing_throughput_units(self):
        task = workload.Task(
            name="bulk-index",
            operation=workload.Operation(name="bulk-index", operation_type=workload.OperationType.Bulk.to_hyphenated_string()),
            clients=4,
        ...
        self.assertEqual("Target throughput for [bulk-index] is specified in [MB/s] but the task throughput is measured in [docs/s].", ex.exception.args[0])
    ...
    def test_scheduler_accepts_differing_units_pages_and_ops(self):
        task = workload.Task(
            name="scroll-query",
            operation=workload.Operation(name="scroll-query", operation_type=workload.OperationType.Search.to_hyphenated_string()),
            clients=1,
            params={
                # implicitly: ops/s
    ...
    def test_scheduler_does_not_change_throughput_for_empty_requests(self):
        task = workload.Task(
            ...
            operation=workload.Operation(name="query", operation_type=workload.OperationType.Search.to_hyphenated_string()),
            clients=1,
            params={
                # implicitly: ops/s
                "target-throughput": 10
...
class LegacyWrappingSchedulerTests(TestCase):
    ...
    def test_legacy_scheduler(self):
        task = workload.Task(
            name="raw-request", operation=workload.Operation(name="raw", operation_type=workload.OperationType.RawRequest.to_hyphenated_string()), clients=1, schedule="simple"
        )
