...
class SourceDownloaderTest(TestCase):
    def setUp(self):
        ...
        self.cluster_config = ClusterConfigInstance(
            ...
            variables={
                "source": {
                    "root": {"dir": "/fake/dir/for/source"},
                    "solr": {"subdir": "solr_sub-dir"},
                    "remote": {"repo": {"url": "https://git.remote.fake"}},
                    "revision": "current",
        ...
        self.solr_source_downloader = SourceDownloader(
            self.cluster_config, self.executor, self.source_repository_provider, self.binary_builder, self.template_renderer, self.artifact_variables_provider
        )
    ...
    def test_download(self):
        ...
        self.source_repository_provider.fetch_repository.assert_has_calls([mock.call(self.host, "https://git.remote.fake", "current", "/fake/dir/for/source/solr_sub-dir")])
        self.binary_builder.build.assert_has_calls([mock.call(self.host, ["fake clean", "fake build"])])
        self.artifact_variables_provider.get_artifact_variables.assert_has_calls([mock.call(self.host)])
        self.template_renderer.render_template_string.assert_has_calls(
            [mock.call("gradle clean", {"OSNAME": "fake_OS"}), mock.call("gradle build", {"OSNAME": "fake_OS"}), mock.call("{{OSNAME}}.tar.gz", {"OSNAME": "fake_OS"})]
        )
