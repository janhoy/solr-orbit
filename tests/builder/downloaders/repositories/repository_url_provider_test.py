...
class RepositoryUrlProviderTest(TestCase):
    def setUp(self):
        ...
        self.host = None
        self.variables = {"distribution": {"version": "1.2.3"}, "fake": {"url": "opensearch/{{VERSION}}/opensearch-{{VERSION}}-{{OSNAME}}-{{ARCH}}.tar.gz"}}
    ...
    def test_get_url(self):
        ...
        self.artifact_variables_provider.get_artifact_variables.assert_has_calls([mock.call(self.host, "1.2.3")])
        self.template_renderer.render_template_string.assert_has_calls([mock.call("opensearch/{{VERSION}}/opensearch-{{VERSION}}-{{OSNAME}}-{{ARCH}}.tar.gz", {"fake": "vars"})])
