...
class BareProvisionerTests(TestCase):
    ...
    def test_prepare_without_plugins(self, mock_rm, mock_ensure_dir, mock_decompress):
        ...
        installer = provisioner.NodeInstaller(
            cluster_config=cluster_config.ClusterConfigInstance(
        ...
        p = provisioner.BareProvisioner(os_installer=installer, apply_config=null_apply_config)
...
class NodeInstallerTests(TestCase):
    ...
    def test_prepare_default_data_paths(self, mock_rm, mock_ensure_dir, mock_decompress):
        installer = provisioner.NodeInstaller(
            cluster_config=cluster_config.ClusterConfigInstance(names="defaults", root_path=None, config_paths="/tmp"),
            java_home="/usr/local/javas/java8",
            node_name="benchmark-node-0",
            all_node_ips=["10.17.22.22", "10.17.22.23"],
            all_node_names=["benchmark-node-0", "benchmark-node-1"],
    ...
    def test_prepare_user_provided_data_path(self, mock_rm, mock_ensure_dir, mock_decompress):
        installer = provisioner.NodeInstaller(
            cluster_config=cluster_config.ClusterConfigInstance(names="defaults", root_path=None, config_paths="/tmp", variables={"data_paths": "/tmp/some/data-path-dir"}),
            java_home="/usr/local/javas/java8",
            node_name="benchmark-node-0",
            all_node_ips=["10.17.22.22", "10.17.22.23"],
            all_node_names=["benchmark-node-0", "benchmark-node-1"],
    ...
    def test_invokes_hook_with_java_home(self):
        installer = provisioner.NodeInstaller(
            cluster_config=cluster_config.ClusterConfigInstance(
                names="defaults", root_path="/tmp", config_paths="/tmp/templates", variables={"data_paths": "/tmp/some/data-path-dir"}
            ),
            java_home="/usr/local/javas/java8",
            node_name="benchmark-node-0",
            all_node_ips=["10.17.22.22", "10.17.22.23"],
...
class DockerProvisionerTests(TestCase):
    ...
    def test_provisioning_with_defaults(self, uuid4):
        ...
        c = cluster_config.ClusterConfigInstance("unit-test-cluster-config-instance", None, "/tmp", variables={"docker_image": "solr"})
...
networks:
  solr-net:"""
    ...
    def test_provisioning_with_variables(self, uuid4):
        ...
        c = cluster_config.ClusterConfigInstance(
            "unit-test-cluster-config-instance", None, "/tmp", variables={"docker_image": "solr", "docker_mem_limit": "256m", "docker_cpu_count": 2}
        )
...
class CleanupTests(TestCase):
    ...
    def test_preserves(self, mock_path_exists, mock_rm):
        ...
        provisioner.cleanup(preserve=True, install_dir="./benchmark/test_runs/install", data_paths=["./benchmark/test_runs/data"])
    ...
    def test_cleanup(self, mock_path_exists, mock_rm):
        ...
        provisioner.cleanup(preserve=False, install_dir="./benchmark/test_runs/install", data_paths=["./benchmark/test_runs/data"])
