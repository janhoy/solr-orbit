...
class ConfigApplierTest(TestCase):
    def setUp(self):
        self.node = Node(
            binary_path="/fake_binary_path", data_paths=["/fake1", "/fake2"], name=None, pid=None, telemetry=None, port=None, root_dir=None, log_path=None, heap_dump_path=None
        )
        self.host = None
    ...
    def test_apply_config_binary_file(self, is_plain_text, os_walk):
        ...
        self.assertEqual(mounts, {"/fake_binary_path/sub_fake_config_path/fake_file": "/var/solr/sub_fake_config_path/fake_file"})
        self.path_manager.create_path.assert_has_calls([mock.call(self.host, "/fake_binary_path/sub_fake_config_path")])
        self.template_renderer.render_template_file.assert_has_calls([])
        self.executor.execute.assert_has_calls([mock.call(self.host, "cp /fake_config_path/sub_fake_config_path/fake_file /fake_binary_path/sub_fake_config_path/fake_file")])
    ...
    def test_apply_config_plaintext_file(self, is_plain_text, os_walk):
        ...
        with patch("builtins.open", mock_open(read_data="fake_data")) as mock_file:
            ...
            self.executor.execute.assert_has_calls([mock.call(self.host, "cp /fake_binary_path/sub_fake_config_path/fake_file /fake_binary_path/sub_fake_config_path/fake_file")])
