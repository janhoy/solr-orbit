...
class JvmTests(TestCase):
    ...
    def test_resolve_path_for_one_version_no_matching_version(self, getenv):
        ...
        with self.assertRaisesRegex(expected_exception=exceptions.SystemSetupError, expected_regex="JAVA_HOME points to JDK 7 but it should point to JDK 8."):
    ...
    def test_resolve_path_for_one_version_no_env_vars_defined(self, getenv):
        ...
        with self.assertRaisesRegex(expected_exception=exceptions.SystemSetupError, expected_regex="Neither JAVA8_HOME nor JAVA_HOME point to a JDK 8 installation."):
            jvm.resolve_path(majors=8, sysprop_reader=self.path_based_prop_version_reader)
