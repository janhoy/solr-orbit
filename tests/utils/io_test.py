...
class IoTests(TestCase):
    ...
    def test_archive(self):
        ...
        # solr-orbit does not recognize .7z
...
class TestDecompression:
    def test_decompresses_supported_file_formats(self):
        for ext in ["zip", "gz", "bz2", "tgz", "tar.bz2", "tar.gz", "zst"]:
            ...
            assert os.path.exists(decompressed_path) is True, f"Could not decompress [{archive_path}] to [{decompressed_path}] (target file does not exist)"
    ...
    def test_decompresses_supported_file_formats_with_lib_as_failover(self, mocked_is_executable):
        for ext in ["zip", "gz", "bz2", "tgz", "tar.bz2", "tar.gz", "zst"]:
            ...
            logger = logging.getLogger("solrorbit.utils.io")
