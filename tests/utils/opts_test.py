...
class ConfigHelperFunctionTests(TestCase):
    ...
    def test_kv_to_map(self):
        ...
        self.assertEqual({"k": "v", "size": 4, "empty": False, "temperature": 0.5}, opts.kv_to_map(["k:'v'", "size:4", "empty:false", "temperature:0.5"]))
...
class GenericHelperFunctionTests(TestCase):
    ...
    def test_make_list_of_close_matches(self):
        word_list = ["bulk_indexing_clients", "bulk_indexing_iterations", "target_throughput", "bulk_size", "number_of-shards", "number_of_replicas", "index_refresh_interval"]
...
class TestTargetHosts(TestCase):
    ...
    def test_csv_hosts_parses(self):
        ...
        self.assertEqual({"default": [{"host": "127.0.0.1", "port": 9200}, {"host": "10.17.0.5", "port": 19200}]}, opts.TargetHosts(target_hosts).all_hosts)

        self.assertEqual([{"host": "127.0.0.1", "port": 9200}, {"host": "10.17.0.5", "port": 19200}], opts.TargetHosts(target_hosts).default)
    ...
    def test_jsonstring_parses_as_dict_of_clusters(self):
        ...
        self.assertEqual(
            {"default": ["127.0.0.1:9200", "10.17.0.5:19200"], "remote_1": ["88.33.22.15:19200"], "remote_2": ["10.18.0.6:19200", "10.18.0.7:19201"]},
            opts.TargetHosts(target_hosts).all_hosts,
        )

    def test_json_file_parameter_parses(self):
        self.assertEqual(
            {"default": ["127.0.0.1:9200", "10.127.0.3:19200"]}, opts.TargetHosts(os.path.join(os.path.dirname(__file__), "resources", "target_hosts_1.json")).all_hosts
        )
...
class TestClientOptions(TestCase):
    def test_csv_client_options_parses(self):
        ...
        self.assertEqual({"use_ssl": True, "verify_certs": True, "ca_certs": "/path/to/cacert.pem"}, opts.ClientOptions(client_options_string).default)

        self.assertEqual({"use_ssl": True, "verify_certs": True, "ca_certs": "/path/to/cacert.pem"}, opts.ClientOptions(client_options_string).default)

        self.assertEqual({"default": {"use_ssl": True, "verify_certs": True, "ca_certs": "/path/to/cacert.pem"}}, opts.ClientOptions(client_options_string).all_client_options)
    ...
    def test_json_file_parameter_parses(self):
        ...
        self.assertEqual({"default": {"timeout": 60}}, opts.ClientOptions(os.path.join(os.path.dirname(__file__), "resources", "client_options_2.json")).all_client_options)
    ...
    def test_no_client_option_parses_to_default(self):
        ...
        self.assertEqual({"default": {"timeout": 60}}, opts.ClientOptions(client_options_string, target_hosts=target_hosts).all_client_options)
    ...
    def test_no_client_option_parses_to_default_with_multicluster(self):
        ...
        self.assertEqual({"default": {"timeout": 60}, "remote": {"timeout": 60}}, opts.ClientOptions(client_options_string, target_hosts=target_hosts).all_client_options)
