...
@pytest.mark.benchmark(group="bulk-runner", warmup="on", warmup_iterations=10000, disable_gc=True)
def test_bulk_runner_without_errors_no_detailed_results(benchmark):
    benchmark(bulk_index, solr_mock, {"action-metadata-present": True, "body": "bulk API body", "bulk-size": BULK_SIZE})


@pytest.mark.benchmark(group="bulk-runner", warmup="on", warmup_iterations=1000, disable_gc=True)
def test_bulk_runner_without_errors_with_detailed_results(benchmark):
    benchmark(bulk_index, solr_mock, {"action-metadata-present": True, "body": "bulk API body", "bulk-size": BULK_SIZE, "detailed-results": True})
