...
from solrorbit import version, actor, config, paths, test_run_orchestrator, publisher, metrics, workload, exceptions, log
...
def create_arg_parser():
    ...
    def add_workload_source(subparser):
        ...
        workload_source_group.add_argument("--workload-path", help="Define the path to a workload.")
        subparser.add_argument("--workload-revision", help="Define a specific revision in the workload repository that solr-orbit should use.", default=None)
    ...
    list_parser.add_argument(
        ...
        help="The configuration for which the tool should show the available options. Possible values are: telemetry, workloads, pipelines, test-runs, cluster-configs",
        choices=["telemetry", "workloads", "pipelines", "test-runs", "aggregated-results", "cluster-configs"],
    )
    list_parser.add_argument(
        "--limit",
    ...
    info_parser.add_argument(
        "--workload-params", "-wp", help="Define a comma-separated list of key:value pairs that are injected verbatim to the workload as variables.", default=""
    )
    info_parser.add_argument("--test-procedure", help=f"Define the test_procedure to use. List possible test_procedures for workloads with `{PROGRAM_NAME} list workloads`.")
    info_task_filter_group = info_parser.add_mutually_exclusive_group()
    info_task_filter_group.add_argument("--include-tasks", help="Defines a comma-separated list of tasks to run. By default all tasks of a test_procedure are run.")
    info_task_filter_group.add_argument("--exclude-tasks", help="Defines a comma-separated list of tasks not to run. By default all tasks of a test_procedure are run.")

    synthetic_data_generator_parser = subparsers.add_parser(
        "generate-data", help="Generate synthetic data based on existing index mappings or custom module." + "This data can be ported into Solr Orbit workloads."
    )

    exclusive_file_inputs = synthetic_data_generator_parser.add_mutually_exclusive_group(required=True)
    exclusive_file_inputs.add_argument("--index-mappings", "-i", help="Index mappings (JSON) to generate synthetic data from.")
    ...
    exclusive_params.add_argument("--total-size", "-s", type=int, help="Total size in GB of synthetically generated data corpora")
    synthetic_data_generator_parser.add_argument("--index-name", "-n", required=True, help="Index name associated with generated corpora")
    synthetic_data_generator_parser.add_argument(
        "--output-path", "-p", default=os.path.join(os.getcwd(), "generated_corpora"), help="Output path for data corpora. Data corpora will be written in a directory."
    )
    synthetic_data_generator_parser.add_argument(
        "--custom-config", "-c", default=None, help="Optional config where users can specify overrides for mapping synthetic data generator or values that module should use."
    )
    synthetic_data_generator_parser.add_argument(
        "--test-document",
        "-t",
    ...
    create_workload_parser.add_argument("--workload", "-w", required=True, help="Name of the generated workload")
    create_workload_parser.add_argument("--indices", "-i", type=non_empty_list, required=True, help="Comma-separated list of indices to include in the workload")
    create_workload_parser.add_argument("--target-hosts", "-t", default="", required=True, help="Comma-separated list of host:port pairs which should be targeted")
    ...
    create_workload_parser.add_argument("--output-path", default=os.path.join(os.getcwd(), "workloads"), help="Workload output directory (default: workloads/)")
    create_workload_parser.add_argument(
        "--custom-queries", type=argparse.FileType("r"), help="Input JSON file to use containing custom workload queries that override the default match_all query"
    )
    create_workload_parser.add_argument(
        "--number-of-docs",
    ...
    convert_workload_parser = subparsers.add_parser("convert-workload", help="Convert an OpenSearch Benchmark workload to Solr-native format")
    convert_workload_parser.add_argument("--workload-path", required=True, help="Path to the source OpenSearch Benchmark workload directory (must contain workload.json).")
    convert_workload_parser.add_argument("--output-path", default=None, help="Path where the converted Solr workload will be written (default: <workload-path>-solr).")
    convert_workload_parser.add_argument("--force", action="store_true", default=False, help="Overwrite an existing converted workload directory.")

    compare_parser = subparsers.add_parser("compare", help="Compare two test_runs")
    compare_parser.add_argument("--baseline", "-b", required=True, help=f"TestRun ID of the baseline (see {PROGRAM_NAME} list test-runs).")
    compare_parser.add_argument("--contender", "-c", required=True, help=f"TestRun ID of the contender (see {PROGRAM_NAME} list test-runs).")
    compare_parser.add_argument(
        "--percentiles",
        help=f"A comma-separated list of percentiles to report latency and service time.(default: {metrics.GlobalStatsCalculator.DEFAULT_LATENCY_PERCENTILES}).",
        default=metrics.GlobalStatsCalculator.DEFAULT_LATENCY_PERCENTILES,
    )
    compare_parser.add_argument(
        "--results-format", help="Define the output format for the command line results (default: markdown).", choices=["markdown", "csv"], default="markdown"
    )
    compare_parser.add_argument(
    ...
    compare_parser.add_argument("--results-file", help="Write the command line results also to the provided file.", default="")
    compare_parser.add_argument("--show-in-results", help="Whether to include the comparison in the results file.", default=True)

    visualize_parser = subparsers.add_parser("visualize", help="Generate HTML visualization for a test run")
    visualize_parser.add_argument("--test-run-id", "-tid", dest="test_run_id", required=True, help=f"TestRun ID to visualize (see {PROGRAM_NAME} list test_runs).")
    ...
    aggregate_parser.add_argument("--test-runs", type=non_empty_list, required=True, help="Comma-separated list of TestRun IDs to aggregate")
    aggregate_parser.add_argument("--test-runs-id", "-tid", help="Define a unique id for this aggregated test-run.", default="")
    aggregate_parser.add_argument("--results-file", help="Write the aggregated results to the provided file.", default="")
    aggregate_parser.add_argument("--workload-repository", help="Define the repository from where solr-orbit will load workloads (default: default).", default="default")

    download_parser = subparsers.add_parser("download", help="Downloads an artifact")
    download_parser.add_argument("--cluster-config-repository", help="Define the repository from where solr-orbit will load cluster-configs (default: default).", default="default")
    download_parser.add_argument("--cluster-config-revision", help="Define a specific revision in the cluster-config repository that solr-orbit should use.", default=None)
    download_parser.add_argument("--cluster-config-path", help="Define the path to the cluster-config and plugin configurations to use.")
    download_parser.add_argument(
        "--distribution-version",
        type=supported_os_version,
        help="Define the version of the distribution to download. Check https://projects.apache.org/project.html?solr for released versions.",
        default="",
    )
    download_parser.add_argument("--distribution-repository", help="Define the repository from where the distribution should be downloaded (default: release).", default="release")
    download_parser.add_argument(
        "--cluster-config",
        help=f"Define the cluster-config to use. List possible cluster-configs with `{PROGRAM_NAME} list cluster-configs` (default: defaults).",
        default="defaults",
    )  # optimized for local usage
    download_parser.add_argument(
        "--cluster-config-params", help="Define a comma-separated list of key:value pairs that are injected verbatim as variables for the cluster-config.", default=""
    )
    ...
    install_parser.add_argument("--build-type", help=argparse.SUPPRESS, choices=["tar", "docker"], default="tar")
    install_parser.add_argument("--cluster-config-repository", help="Define the repository from where solr-orbit will load cluster-configs (default: default).", default="default")
    install_parser.add_argument("--cluster-config-revision", help="Define a specific revision in the cluster-config repository that solr-orbit should use.", default=None)
    install_parser.add_argument("--cluster-config-path", help="Define the path to the cluster-config and plugin configurations to use.")
    install_parser.add_argument("--runtime-jdk", type=runtime_jdk, help="The major version of the runtime JDK to use during installation.", default=None)
    install_parser.add_argument("--distribution-repository", help="Define the repository from where the distribution should be downloaded (default: release).", default="release")
    install_parser.add_argument(
        "--distribution-version",
        type=supported_os_version,
        help="Define the version of the distribution to download. Check https://archive.apache.org/dist/solr/solr/ for released versions.",
        default="",
    )
    install_parser.add_argument(
        "--cluster-config",
    ...
    install_parser.add_argument(
        "--solr-modules", help="Comma-separated list of Solr modules to enable (sets SOLR_MODULES). Example: --solr-modules=analytics,extraction", default=""
    )
    install_parser.add_argument("--plugin-params", help="Define a comma-separated list of key:value pairs that are injected verbatim to all plugins as variables.", default="")
    install_parser.add_argument("--network-host", help="The IP address to bind to and publish", default="127.0.0.1")
    install_parser.add_argument("--http-port", help="The port to expose for HTTP traffic", default="38983")
    install_parser.add_argument("--node-name", help="The name of this Solr node", default="benchmark-node-0")
    install_parser.add_argument("--master-nodes", help="A comma-separated list of the initial master node names", default="")
    install_parser.add_argument("--seed-hosts", help="A comma-separated list of the initial seed host IPs", default="")
    ...
    start_parser.add_argument("--test-run-id", "-tid", required=True, help="Define a unique id for this test_run.", default="")
    start_parser.add_argument("--runtime-jdk", type=runtime_jdk, help="The major version of the runtime JDK to use.", default=None)
    start_parser.add_argument(
        "--telemetry",
        help=f"Enable the provided telemetry devices, provided as a comma-separated list. List possible telemetry devices with `{PROGRAM_NAME} list telemetry`.",
        default="",
    )
    start_parser.add_argument(
        "--telemetry-params", help="Define a comma-separated list of key:value pairs that are injected verbatim to the telemetry devices as parameters.", default=""
    )
    ...
    stop_parser.add_argument(
        "--preserve-install", help=f"Keep the benchmark candidate and its index. (default: {str(preserve_install).lower()}).", default=preserve_install, action="store_true"
    )

    for p in [list_parser, test_run_parser]:
        p.add_argument("--distribution-version", type=supported_os_version, help="Define the version of the distribution to download.", default="")
        p.add_argument("--cluster-config-path", help="Define the path to the cluster-config and plugin configurations to use.")
        p.add_argument("--cluster-config-repository", help="Define repository from where solr-orbit will load cluster-configs (default: default).", default="default")
        p.add_argument("--cluster-config-revision", help="Define a specific revision in the cluster-config repository that solr-orbit should use.", default=None)

    test_run_parser.add_argument("--test-run-id", "-tid", help="Define a unique id for this test-run.", default=str(uuid.uuid4()))
    ...
    test_run_parser.add_argument("--workload", "-w", help=f"Define the workload to use. List possible workloads with `{PROGRAM_NAME} list workloads`.")
    test_run_parser.add_argument(
        "--workload-params", "-wp", help="Define a comma-separated list of key:value pairs that are injected verbatim to the workload as variables.", default=""
    )
    test_run_parser.add_argument("--test-procedure", help=f"Define the test_procedure to use. List possible test_procedures for workloads with `{PROGRAM_NAME} list workloads`.")
    test_run_parser.add_argument(
        "--cluster-config",
        help=f"Define the cluster-config to use. List possible cluster-configs with `{PROGRAM_NAME} list cluster-configs` (default: defaults).",
        default="defaults",
    ...
    test_run_parser.add_argument("--runtime-jdk", type=runtime_jdk, help="The major version of the runtime JDK to use.", default=None)
    test_run_parser.add_argument(
        "--solr-modules", help="Comma-separated list of Solr modules to enable (sets SOLR_MODULES). Example: --solr-modules=analytics,extraction", default=""
    )
    test_run_parser.add_argument("--plugin-params", help="Define a comma-separated list of key:value pairs that are injected verbatim to all plugins as variables.", default="")
    test_run_parser.add_argument(
        "--target-hosts",
        "-t",
        help="Define a comma-separated list of host:port pairs which should be targeted if using the pipeline 'benchmark-only' (default: localhost:9200).",
        default="",
    )  # actually the default is pipeline specific and it is set later
    test_run_parser.add_argument("--worker-ips", help="Define a comma-separated list of hosts which should generate load (default: localhost).", default="localhost")
    test_run_parser.add_argument("--grpc-target-hosts", help="Define a comma-separated list of host:port pairs for gRPC endpoints (default: localhost:9400).", default="")
    test_run_parser.add_argument(
        "--client-options",
        "-c",
        help=f"Define a comma-separated list of client options to use. The options will be passed to the benchmark client (default: {opts.ClientOptions.DEFAULT_CLIENT_OPTIONS}).",
        default=opts.ClientOptions.DEFAULT_CLIENT_OPTIONS,
    )
    test_run_parser.add_argument("--on-error", choices=["continue", "abort"], help="Controls how solr-orbit behaves on response errors (default: continue).", default="continue")
    test_run_parser.add_argument(
        "--telemetry",
        help=f"Enable the provided telemetry devices, provided as a comma-separated list. List possible telemetry devices with `{PROGRAM_NAME} list telemetry`.",
        default="",
    ...
    test_run_parser.add_argument("--distribution-repository", help="Define the repository from where the distribution should be downloaded (default: release).", default="release")

    task_filter_group = test_run_parser.add_mutually_exclusive_group()
    task_filter_group.add_argument("--include-tasks", help="Defines a comma-separated list of tasks to run. By default all tasks of a test_procedure are run.")
    task_filter_group.add_argument("--exclude-tasks", help="Defines a comma-separated list of tasks not to run. By default all tasks of a test_procedure are run.")
    test_run_parser.add_argument(
        "--user-tag",
        help="Define a user-specific key-value pair (separated by ':'). It is added to each metric record as meta info. Example: intention:baseline-ticket-12345",
        default="",
    )
    test_run_parser.add_argument(
        "--results-format", help="Define the output format for the command line results (default: markdown).", choices=["markdown", "csv"], default="markdown"
    ...
    test_run_parser.add_argument("--results-file", help="Write the command line results also to the provided file.", default="")
    test_run_parser.add_argument(
        "--preserve-install", help=f"Keep the benchmark candidate and its index. (default: {str(preserve_install).lower()}).", default=preserve_install, action="store_true"
    )
    test_run_parser.add_argument(
    ...
    test_run_parser.add_argument("--enable-assertions", help="Enables assertion checks for tasks (default: false).", default=False, action="store_true")
    test_run_parser.add_argument(
        "--kill-running-processes", "-k", action="store_true", default=False, help="If any processes is running, it is going to kill them and allow solr-orbit to continue to run."
    )
    test_run_parser.add_argument(
        "--latency-percentiles",
        help=f"A comma-separated list of percentiles to report for latency (default: {metrics.GlobalStatsCalculator.DEFAULT_LATENCY_PERCENTILES}).",
        default=metrics.GlobalStatsCalculator.DEFAULT_LATENCY_PERCENTILES,
    )
    test_run_parser.add_argument(
    ...
    test_run_parser.add_argument("--randomization-enabled", help="Runs the given workload with query randomization enabled (default: false).", default=False, action="store_true")
    test_run_parser.add_argument(
        "--randomization-repeat-frequency",
        help=f"The repeat_frequency for query randomization. Ignored if randomization is off(default: {workload.loader.QueryRandomizerWorkloadProcessor.DEFAULT_RF}).",
    ...
    test_run_parser.add_argument("--test-iterations", help="The number of times to run the workload (default: 1).", default=1)
    test_run_parser.add_argument(
        "--aggregate", type=lambda x: str(x).lower() in ["true", "1", "yes", "y"], help="Aggregate the results of multiple test runs (default: true).", default=True
    )
    test_run_parser.add_argument("--sleep-timer", help="Sleep for the specified number of seconds before starting the next test run (default: 5).", default=5)
    test_run_parser.add_argument(
        "--cancel-on-error",
        action="store_true",
        help="Stop running tests if an error occurs in one of the test iterations (default: false).",
    )
    test_run_parser.add_argument("--load-test-qps", help="Run a load test on your cluster, up to a certain QPS value (default: 0)", default=0)
    test_run_parser.add_argument(
        "--redline-test",
    ...
    test_run_parser.add_argument("--redline-scale-step", type=int, help="How many clients to add while scaling up during redline testing (default: 5).", default=None)
    test_run_parser.add_argument("--redline-scaledown-percentage", type=float, help="What percentage of clients to remove when errors occur (default: 10%%).", default=None)
    test_run_parser.add_argument(
        "--redline-post-scaledown-sleep", type=int, help="How many seconds to wait before scaling up again after a scale down (default: 30).", default=None
    )
    test_run_parser.add_argument(
        "--redline-max-clients",
        type=int,
    ...
    test_run_parser.add_argument(
        "--redline-max-cpu-usage", type=int, help="Maximum CPU utilization before scaling back client numbers. Used to activate CPU-based feedback in solr-orbit.", default=None
    )
    test_run_parser.add_argument(
    ...
    test_run_parser.add_argument(
        "--redline-cpu-check-interval", type=int, help="How many seconds between CPU checks there should be during CPU-based redline testing. (Default: 30)", default=None
    )
    test_run_parser.add_argument(
        "--visualize", help="Generate HTML visualizations for benchmark results. Stored in the test runs directory by default", action="store_true", default=False
    )
    test_run_parser.add_argument(
    ...
    test_run_parser.add_argument("--effective-start-date", help=argparse.SUPPRESS, type=lambda s: datetime.datetime.strptime(s, "%Y-%m-%d %H:%M:%S"), default=None)
    # Skips checking that the REST API is available before proceeding with the benchmark
    test_run_parser.add_argument("--skip-rest-api-check", help=argparse.SUPPRESS, action="store_true", default=False)
    ...
    for p in [
        ...
        stop_parser,
        info_parser,
        synthetic_data_generator_parser,
        ...
        p.add_argument("--quiet", help="Suppress as much as output as possible (default: false).", default=False, action="store_true")
        p.add_argument("--offline", help="Assume that solr-orbit has no connection to the Internet (default: false).", default=False, action="store_true")
...
def dispatch_visualize(cfg):
    ...
    try:
        ...
        html_path = store.file_store.store_html_results(te) if isinstance(store, metrics.CompositeTestRunStore) else store.store_html_results(te)
...
def print_help_on_errors():
    ...
    console.println(f"* Raise an issue in the project issue tracker and include the log files in {paths.logs()}.")
...
def run_test(cfg, kill_running_processes=False):
    ...
    if kill_running_processes:
        ...
        try:
            ...
            logger.exception("Could not terminate potentially running solr-orbit instances correctly. Attempting to go on anyway.")
    else:
        ...
        if other_benchmark_processes:
            ...
            msg = (
                f"There are other solr-orbit processes running on this machine (PIDs: {pids}) but only one "
        ...
        try:
            if isinstance(store, metrics.InMemoryMetricsStore):
                raise exceptions.SystemSetupError(
                    "CPU-based feedback requires a metrics store, but you're using the in-memory store. Specify a metrics store in your benchmark.ini or via CLI to continue."
                )
        finally:
            store.close()
...
def with_actor_system(runnable, cfg):
    ...
    # This happens when the admin process could not be started, e.g. because it could not open a socket.
        ...
        if str(e) == "Unable to determine valid external socket address.":
            console.warn("Could not determine a socket address. Are you running without any network? Switching to degraded mode.", logger=logger)
            logger.info("Falling back to offline actor system.")
            actor.use_offline_actor_system()
            actors = actor.bootstrap_actor_system(try_join=False, prefer_local_only=True)
        else:
    ...
    try:
        ...
        if not already_running:
            ...
            if not shutdown_complete and times_interrupted > 0:
                ...
                console.warn("Could not terminate all internal processes within timeout. Please check and force-terminate all solr-orbit processes.")
...
def configure_builder_params(args, cfg, command_requires_cluster_config=True):
    if args.cluster_config_path:
        cfg.add(config.Scope.applicationOverride, "builder", "cluster_config.path", os.path.abspath(io.normalize_path(args.cluster_config_path)))
    ...
    if command_requires_cluster_config:
        ...
        cfg.add(config.Scope.applicationOverride, "builder", "cluster_config.names", opts.csv_to_list(args.cluster_config))
        cfg.add(config.Scope.applicationOverride, "builder", "cluster_config.params", opts.to_dict(args.cluster_config_params))
        ...
        if pipeline == "benchmark-only" and args.cluster_config != "defaults":
            raise SystemExit(
                "ERROR: --cluster-config is only valid for provisioning pipelines (from-distribution, docker, from-sources). It cannot be used with the 'benchmark-only' pipeline."
            )
...
def configure_test(arg_parser, args, cfg):
    ...
    cfg.add(config.Scope.applicationOverride, "worker_coordinator", "worker_ips", opts.csv_to_list(args.worker_ips))
    ...
    try:
        if sub_command == "compare":
            ...
            if workload_converter.is_already_converted(output_dir) and not force:
                console.info(f"Workload already converted at: {output_dir}\nUse --force to overwrite.")
                return True
...
def main():
    ...
    if not args.offline:
        ...
        if not net.has_internet_connection(probing_url):
            console.warn("No Internet connection detected. Automatic download of workload data sets etc. is disabled.", logger=logger)
