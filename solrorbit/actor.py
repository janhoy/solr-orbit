...
class BenchmarkActor(thespian.actors.ActorTypeDispatcher):
    ...
    def transition_when_all_children_responded(self, sender, msg, expected_status, new_status, transition):
        ...
        if self.is_current_status_expected(expected_status):
            ...
            self.logger.debug("[%d] of [%d] child actors have responded for transition from [%s] to [%s].", response_count, expected_count, self.status, new_status)
            if response_count == expected_count:
                self.logger.debug("All [%d] child actors have responded. Transitioning now from [%s] to [%s].", expected_count, self.status, new_status)
