...
from solrorbit import PROGRAM_NAME, exceptions, paths
...
def migrate(config_file, current_version, target_version, out=print, i=input):
    ...
    if current_version < Config.EARLIEST_SUPPORTED_VERSION:
        raise exceptions.ConfigError(f"The config file in {config_file.location} is too old. Please delete it and reconfigure from scratch with {PROGRAM_NAME} configure.")
