...
def resolve_path(majors, sysprop_reader=system_property):
    ...
    if isinstance(majors, int):
        ...
        raise exceptions.SystemSetupError("Install a JDK with one of the versions {} and point to it with one of {}.".format(majors, _checked_env_vars(majors)))
...
def _resolve_single_path(major, mandatory=True, sysprop_reader=system_property):
    ...
    if java_home:
        ...
        if java_home:
            ...
            raise exceptions.SystemSetupError("Neither {} nor {} point to a JDK {} installation.".format(specific_env_var, generic_env_var, major))
        else:
            return None
