...
def _build_gcs_object_url(bucket_name, bucket_path):
    ...
    return functools.reduce(
        ...
        ["https://storage.googleapis.com/storage/v1/b/", f"{quote(bucket_name.strip('/'), safe='')}/", "o/", f"{quote(bucket_path.strip('/'), safe='')}", "?alt=media"],
    )
...
def download_http(url, local_path, expected_size_in_bytes=None, progress_indicator=None):
    with __http().request("GET", url, preload_content=False, retries=10, timeout=urllib3.Timeout(connect=45, read=240)) as r, open(local_path, "wb") as out_file:
        if r.status > 299:
...
def _add_url_param(url, params):
    ...
    query.update(params)
    return urlunparse((url_parsed.scheme, url_parsed.netloc, url_parsed.path, url_parsed.params, urlencode(query, doseq=True), url_parsed.fragment))
    ...
    try:
        ...
        if expected_size_in_bytes is not None and download_size != expected_size_in_bytes:
            ...
            raise exceptions.DataError(
                "Download of [%s] is corrupt. Downloaded [%d] bytes but [%d] bytes are expected. Please retry." % (local_path, download_size, expected_size_in_bytes)
            )
