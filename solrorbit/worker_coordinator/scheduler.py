...
"""
The scheduler module defines an API to determine *when* Solr Orbit should issue a request. There are two types of schedulers:
...
return the next point in time when Solr Orbit should issue a request.
...
* ``before_request`` is invoked by Solr Orbit before a request is executed. ``now`` provides the current timestamp in seconds.
* Similarly, ``after_request`` is invoked by Solr Orbit after a response has been received. ``now`` is the current timestamp,
  ``weight``, ``unit`` and ``request_meta_data`` are passed from the respective runner. For a bulk request, Solr Orbit
...
If the scheduler also needs access to the parameter source, provide a ``parameter_source`` property. Solr Orbit injects the
                ...
                if expected_unit == "ops/s":
                    ...
                    if self.first_request:
                        logging.getLogger(__name__).warning(
                            "Task [%s] throttles based on [%s] but results [%s]. Please specify the target throughput in [%s] instead.",
                            self.task,
                            expected_unit,
                            actual_unit,
                            actual_unit,
                    ...
                    raise exceptions.BenchmarkAssertionError(
                        f"Target throughput for [{self.task}] is specified in [{expected_unit}] but the task throughput is measured in [{actual_unit}]."
                    )
